...
namespace Generation
{
	...
	std::string Generate(LexA::Tables tables)
	{
		...
		for (int i = 0; i < idtable.size; i++)
		{
			...
			{
				if (lextable.table[idtable.table[i].idxfirstLE].lexema == LEX_MAJOR)
					isMain = true;
		...
		for (int i = indexInLexTable; i > 0; i++)
		{
			...
			case LEX_RIGHTBRACE:
				...
				if (IsCheck)
					...
					{
						if (lextable.table[i + 1].lexema != LEX_EITHER) {
							if (thereisnot == false) {
								buffstr += (std::string)ASMWHENNOT + std::to_string(countOfChecks - 1) + " :\n";
							}
							else {
								buffstr += (std::string)ASMWHEN + std::to_string(countOfChecks - 1) + " :\n";
							}
							IsCheck = thereisnot = false;
						}
						else {
							thereisnot = true;
							buffstr += "jmp " + (std::string)ASMWHEN + std::to_string(countOfChecks - 1) + "\n";
							buffstr += (std::string)ASMWHENNOT + std::to_string(countOfChecks - 1) + " :\n";
						}
					}
				break;
			...
			case LEX_WHENEND:
				...
				buffstr += (std::string)ASMWHENNOT + std::to_string(countOfChecks++) + "\n";
				IsCheckForIDAndLITERALS = false;
				thereisnot = false;
				break;
			case LEX_EQUAL:
			{
				if (lextable.table[i - 3].lexema == LEX_WHEN)
					break;
				...
				while (lextable.table[i].lexema != LEX_SEMICOLON && lextable.table[i].lexema != LATTICE)
				{
					...
					case LEX_LITERAL:
					{
						if (lextable.table[i].value == LEX_LIBFUNCTION) {
							...
							case -2:
								buffstr += "\tpush range\n";
								MainStack.push("range");
								break;
							case -3:
								buffstr += "\tpush copyline\n";
								MainStack.push("copyline");
							default:
								break;
							}
							break;
