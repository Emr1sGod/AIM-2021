...
namespace IT
{
	...
	void showTable(IdTable& idtable, std::ofstream& file)                     //функция вывода 
	{
		...
		file << std::setfill(' ') << std::setw(60) << "ID TABLE" <<  "\n\n";              //вывод преамбулы
		file << std::setfill(' ') << std::setw(18) << "Name" << std::setw(18) << "Type" << std::setw(18) << "Data type" <<
			std::setw(18) << "First in LT" << std::setw(27) << "Value"  << std::setw(16) << "Visibility" << "\n\n";

		for (int i = 1; i < idtable.size; i++)                                 //вывод таблицы
		{
			iddatatype = idtable.table[i].iddatatype;
			idtype = idtable.table[i].idtype;
			file << std::setfill(' ') << std::setw(3) << i;
			file << std::setfill(' ') << std::setw(15) << idtable.table[i].id << std::setw(20);
			...
			{
			case 1:
				file << std::setfill(' ') << "brief" << std::setw(15);
				break;
			case 2:
				file << std::setfill(' ') << "line" << std::setw(15);
				break;
			}

			file << std::setfill(' ') << idtable.table[i].idxfirstLE << std::setw(30);
