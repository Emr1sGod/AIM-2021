...
namespace Semantic
{
	void CheckSemanticAfterPolish(LexA::Tables myTables)
	{
		...
		for (int i = 0; i < myTables.mylextable.size; i++) {       // семантика проверки
			...
			{
				if (myTables.mylextable.table[i - 3].lexema == LEX_WHEN) {
	...
	{
		std::string mainFunctionStrName = "major";           //проверка на существованик функции main
		...
		for (int i = 0; i < myTables.mylextable.size; i++) {       // семантика проверки
			...
			if (myTables.mylextable.table[i].lexema == LEX_EQUAL && myTables.mylextable.table[i-3].lexema != LEX_WHEN)
			{
				int k = i;
				int counter = 0;
			...
			if (myTables.mylextable.table[i].idxTI != LT_TI_NULLIDX)    
			{
				...
				if (myTables.myidtable.table[myTables.mylextable.table[i].idxTI].idtype == IT::F && myTables.mylextable.table[i].lexema != LEX_MAJOR)
				{
					int k = i;
					int counterMain = 0;
					int counterObs = 0;
